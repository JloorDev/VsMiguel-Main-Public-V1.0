...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'stageDay': //StageDay
					bgDay = new FlxSprite(-600, -200);
					bgDay.frames = Paths.getSparrowAtlas('vsMiguel/stageDay/bgDay', 'shared');
					bgDay.animation.addByPrefix('idle', 'BGHouseDay', 24);
					bgDay.animation.play('idle');
					bgDay.setGraphicSize(Std.int(bgDay.width * 1.1));
					bgDay.scrollFactor.set(0.9, 0.9);
					bgDay.updateHitbox();
					bgDay.antialiasing = true;
					add(bgDay);

					people = new FlxSprite(-600, -200);
					people.frames = Paths.getSparrowAtlas('vsMiguel/stageDay/CharsBGSong1', 'shared');
					people.animation.addByPrefix('idle', 'CharsDayAnimation', 24);
				...
				case 'stageAfternoon': //stageAfternoon
					...
					GameOverSubstate.loopSoundName = 'gameOver';
					GameOverSubstate.endSoundName = 'gameOverEnd';
					GameOverSubstate.characterName = 'bf-afternoon';

					bgAfterNoon = new FlxSprite(-600, -200);
					bgAfterNoon.frames = Paths.getSparrowAtlas('vsMiguel/stageAfternoon/bgAfternoon', 'shared');
					bgAfterNoon.animation.addByPrefix('idle', 'BGHouseAfterNoon', 24);
					bgAfterNoon.animation.play('idle');
					bgAfterNoon.setGraphicSize(Std.int(bgAfterNoon.width * 1.1));
					bgAfterNoon.scrollFactor.set(0.9, 0.9);
					bgAfterNoon.updateHitbox();
					bgAfterNoon.antialiasing = true;
					add(bgAfterNoon);

					people = new FlxSprite(-600, -200);
					people.frames = Paths.getSparrowAtlas('vsMiguel/stageAfternoon/CharsBGSong2', 'shared');
					people.animation.addByPrefix('idle', 'CharsÁftermoonAnimation', 24);
					people.animation.play('idle');
					people.antialiasing = true;
					people.scrollFactor.set(0.9, 0.9);
					people.x += 400;
				...
				case 'stageNight': //stageNight
					...
					people = new FlxSprite(-600, -200);
					people.frames = Paths.getSparrowAtlas('vsMiguel/stageNight/CharsBGSong3', 'shared');
					people.animation.addByPrefix('idle', 'CharsNightAnimation', 24);
					people.animation.play('idle');
					people.antialiasing = true;
					people.scrollFactor.set(0.9, 0.9);
					people.x += 400;
				...
				case 'stageNightDark': //stageNightDark
					...
					bgDark.scrollFactor.set(0.9, 0.9);
					bgDark.active = false;
					add(bgDark);
					bgDark.kill();

					mist = new FlxSprite(-600, -200);
					mist.frames = Paths.getSparrowAtlas('vsMiguel/stageDark/BGMist', 'shared');
					mist.animation.addByPrefix('idle', 'BGMist', 24);
					mist.animation.play('idle');
					mist.antialiasing = true;
	...
	override function stepHit()
	{
		...
		if (curStage == 'stageAfternoon' && curStep != stepOfLast)
		{
			switch (curStep)
			{
				case 83:
		...
		if (curStage == 'stageSus' && curSong == 'Brother SUS')
		{
			switch (curStep)
			{
				case 63:
					FlxTween.tween(blackBG, {alpha: 0}, 2.5, {
					...
					FlxTween.tween(blackFG, {alpha: 0}, 2.5, {
					ease: FlxEase.quadOut,
					onComplete: function(twn:FlxTween)
					{
				...
				FlxTween.tween(dad, {alpha: 1}, 2.5, {
					ease: FlxEase.quadOut,
					onComplete: function(twn:FlxTween)
					{
						dad.visible = true;
		...
		if (curStage == 'stageNightDark' && curSong == 'Hungry Dark')
		{
			switch (curStep)
			{
				case 355:
